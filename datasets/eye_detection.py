...
class LFPW(Dataset):
    ...
    def __create_info_df_csv(self):
        ...
        if os.path.exists(self.info_df_csv_file):
            ...
            digit_struct_data = LFPW.read_digit_mat(os.path.join(mat_file_path))
            ...
            self.info_df['img_width'], self.info_df['img_height'] = \
                zip(*self.info_df['img_name'].apply(lambda name: LFPW.get_image_size(
    ...
    def __gen_datasets(self):
        ...
        for _, row in self.info_df.iterrows():
            ...
            increase = round(square_bb_width * LFPW.expansion_ratio / 2.)
    ...
    def read_digit_mat(mat_file):
        ...
        for j in range(f['/digitStruct/bbox'].shape[0]):
            img_name = LFPW.get_name(j, f)
            row_dict = LFPW.get_bbox(j, f)
...
def LFPW_get_datasets(data, load_train=True, load_test=True, resize_size=(96, 96), fold_ratio=2,
                      simplified=False):

    """ Returns LFPW Dataset
    """
    (data_dir, args) = data
    ...
    if load_train:
        ...
        train_dataset = LFPW(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        ...
        test_dataset = LFPW(root_dir=data_dir, d_type='test',
...
def LFPW_74_get_datasets(data, load_train=True, load_test=True):
    """ Returns LFPW Dataset with 96x96 images
    """
    return LFPW_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1)


def LFPW_74_simplified_get_datasets(data, load_train=True, load_test=True):
    """ Returns LFPW Dataset with 96x96 images and simplified labels: 1 for every digit
    """
    return LFPW_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1,
                             simplified=True)
...
datasets = [
   {
       ...
       'input': (3, 74, 74),
       'output': (1, 2),
       'loader': LFPW_74_get_datasets,
       'collate': LFPW.collate_fn
   },
   {
       'name': 'LFPW_74_simplified',
       'input': (3, 74, 74),
       'output': ([1]),
       'loader': LFPW_74_simplified_get_datasets,
       'collate': LFPW.collate_fn
   }
]
