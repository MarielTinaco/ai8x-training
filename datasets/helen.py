...
class HELEN(Dataset):
    ...
    def __create_info_df_csv(self):
        ...
        if os.path.exists(self.info_df_csv_file):
            ...
            digit_struct_data = HELEN.read_digit_mat(os.path.join(mat_file_path))
            ...
            self.info_df['img_width'], self.info_df['img_height'] = \
                zip(*self.info_df['img_name'].apply(lambda name: HELEN.get_image_size(
    ...
    def __gen_datasets(self):
        ...
        for _, row in self.info_df.iterrows():
            ...
            increase = round(square_bb_width * HELEN.expansion_ratio / 2.)
    ...
    def collate_fn(batch):
        ...
        for b in batch:
            if np.shape(b[0]) == torch.Size([3, 74, 74]):
                images.append(b[0])
    ...
    def read_digit_mat(mat_file):
        ...
        for j in range(f['/digitStruct/bbox'].shape[0]):
            img_name = HELEN.get_name(j, f)
            row_dict = HELEN.get_bbox(j, f)
...
def HELEN_get_datasets(data, load_train=True, load_test=True, resize_size=(96, 96), fold_ratio=2,
                      simplified=False):

    """ Returns HELEN Dataset
    """
    (data_dir, args) = data
    ...
    if load_train:
        ...
        train_dataset = HELEN(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        ...
        test_dataset = HELEN(root_dir=data_dir, d_type='test',
...
def HELEN_74_get_datasets(data, load_train=True, load_test=True):
    """ Returns HELEN Dataset with 96x96 images
    """
    return HELEN_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1)


def HELEN_74_simplified_get_datasets(data, load_train=True, load_test=True):
    """ Returns HELEN Dataset with 96x96 images and simplified labels: 1 for every digit
    """
    return HELEN_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1,
                             simplified=True)
...
datasets = [
   {
       ...
       'input': (3, 74, 74),
       'output': (1, 2),
       'loader': HELEN_74_get_datasets,
       'collate': HELEN.collate_fn
   },
   {
       'name': 'HELEN_74_simplified',
       'input': (3, 74, 74),
       'output': ([1]),
       'loader': HELEN_74_simplified_get_datasets,
       'collate': HELEN.collate_fn
   },
   {
        'name': 'HELEN_74_complete',
        'input' : (3, 74, 74),
        'output': (1, 2, 3, 4),
        'loader': HELEN_74_get_datasets,
        'collate': HELEN.collate_fn
